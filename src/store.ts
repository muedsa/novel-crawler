...
const getAndValidBaseConfig = async () => {
  ...
  if (!config.chapterListUrlTemplate) throw new Error('Novel config chapterListUrlTemplate not found');
  if (!config.novels || config.novels.length <= 0) throw new Error('Novel config novels not found');
  ...
  if (!config.nextPageUrlOfChapterSelector) throw new Error('Novel config nextPageUrlOfChapterSelector not found');
  return config;
}
...
const saveNovelChapterPart = async (novelId: string, chapterPartId: string, chapterPart: NovelChapterPart) => 
  await chaptersStore.setValue<NovelChapterPart>(`${novelId}_${chapterPartId}`, chapterPart);
